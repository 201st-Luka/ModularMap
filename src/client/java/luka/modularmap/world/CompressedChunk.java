/*
 * ModularMap
 * Copyright (c) 2024 201st-Luka
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published
 * by the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 */

package luka.modularmap.world;

import net.minecraft.block.BlockState;
import net.minecraft.util.math.BlockPos;
import net.minecraft.world.chunk.Chunk;

import java.util.Vector;

public class CompressedChunk {
    private final Vector<CompressedBlock> blocks = new Vector<>();

    public CompressedChunk(Chunk chunk) {
        for (int x = chunk.getPos().x * 16; x < (chunk.getPos().x + 1) * 16; x++)
            for (int z = chunk.getPos().z * 16; z < (chunk.getPos().z + 1) * 16; z++)
                for (int y = chunk.getTopY() - 1; y >= chunk.getBottomY(); y--) {
                    BlockState blockState = chunk.getBlockState(new BlockPos(x, y, z));
                    if (!blockState.isAir() && (blockState.isOpaque() || blockState.getFluidState().getFluid() != null))
                        blocks.add(new CompressedBlock(blockState.getBlock(), x, z));
                }
    }

    public CompressedBlock[] getBlocks() {
        return blocks.toArray(new CompressedBlock[0]);
    }
}
